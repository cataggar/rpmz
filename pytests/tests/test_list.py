...
def helper_test_list_sub_cmd(utils, sub_cmd):
    ret = utils.run(['rpmz', 'list', sub_cmd])
    assert ret['retval'] == 0
    ret = utils.run(['rpmz', 'list', sub_cmd, utils.config["sglversion_pkgname"]])
    assert ret['retval'] == 0
    ret = utils.run(['rpmz', 'list', sub_cmd, 'invalid_package'])
...
def test_list_top(utils):
    ...
    ret = utils.run(['rpmz', 'list'])
    assert ret['retval'] == 0
    ret = utils.run(['rpmz', 'list', utils.config["sglversion_pkgname"]])
    assert ret['retval'] == 0
    ret = utils.run(['rpmz', 'list', 'invalid_package'])
    assert ret['retval'] == 1011
