...
def generate_config_cleanreq(utils, newconfig, value):
    shutil.copy(os.path.join(utils.config['repo_path'], 'rpmz.conf'), newconfig)
...
def test_autoremove2(utils):
    ...
    utils.run(['rpmz', '-y', 'autoremove', pkgname2])
...
def test_autoremove_conf_true(utils):
    conffile = os.path.join(CONFDIR, 'rpmz.conf')
    ...
    utils.run(['rpmz', '-y', '-c', conffile, 'remove', pkgname])
...
def test_autoremove_conf_noautoremove(utils):
    ...
    utils.run(['rpmz', '-y', '-c', conffile, '--noautoremove', 'remove', pkgname])
...
def test_autoremove_conf_false_autoremove(utils):
    ...
    utils.run(['rpmz', '-y', '-c', conffile, 'autoremove', pkgname])
...
def test_autoremove_noargs(utils):
    ...
    utils.run(['rpmz', '-y', 'remove', '--noautoremove', pkgname])
    assert utils.check_package(pkgname_req)

    utils.run(['rpmz', '-y', 'autoremove'])
    assert not utils.check_package(pkgname)
