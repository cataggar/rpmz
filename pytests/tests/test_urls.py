...
def test_urls_basic(utils):
    ...
    ret = utils.run(['rpmz', 'install', '-y', '--urls', pkgname])
    assert ret['retval'] == 0
...
def test_urls_alldeps(utils):
    ...
    ret = utils.run(['rpmz', 'install', '-y', '--urls', '--alldeps', pkgname])
    assert ret['retval'] == 0
    assert not utils.check_package(pkgname)
...
def test_urls_nodeps(utils):
    ...
    ret = utils.run(['rpmz', 'install', '-y', '--urls', '--nodeps', pkgname])
    assert ret['retval'] == 0
    assert not utils.check_package(pkgname)
...
def test_urls_alldeps_requires_urls_or_downloadonly(utils):
    pkgname = utils.config['sglversion_pkgname']
    ret = utils.run(['rpmz', 'install', '-y', '--alldeps', pkgname])
    assert ret['retval'] != 0
...
def test_urls_nodeps_requires_urls_or_downloadonly(utils):
    pkgname = utils.config['sglversion_pkgname']
    ret = utils.run(['rpmz', 'install', '-y', '--nodeps', pkgname])
    assert ret['retval'] != 0
