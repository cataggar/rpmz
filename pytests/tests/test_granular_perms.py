...
def setup_test(utils):
    ...
    if not utils.config.get('installed', False):
        ...
        rpmz_bin = os.path.join(bin_dir, 'rpmz')
        ...
        path += [bin_dir, test_dir, lib_dir, rpmz_bin]
...
def test_check(utils):
    run_rpmz_cmd(utils, ['check'], 'ignore')


def test_check_update(utils):
    run_rpmz_cmd(utils, ['check-update'], 0)


def test_count(utils):
    run_rpmz_cmd(utils, ['count'], 0)


def test_help(utils):
    run_rpmz_cmd(utils, ['help'], 0)


def test_info(utils):
    utils.run(['rpmz', 'install', '-y', pkgname])
    run_rpmz_cmd(utils, ['info', pkgname], 0)


def test_list(utils):
    run_rpmz_cmd(utils, ['list', 'installed'], 0)


def test_provides(utils):
    run_rpmz_cmd(utils, ['provides', pkgname], 0)


def test_whatprovides(utils):
    run_rpmz_cmd(utils, ['whatprovides', pkgname], 0)


def test_repolist(utils):
    run_rpmz_cmd(utils, ['repolist', 'all'], 0)


def test_reposync(utils):
    dwld_path = '/tmp/rpmz-tmp'
    run_bash_cmd(['chmod', '777', '/tmp'], 0, bash_cmd=True)
    run_bash_cmd(['mkdir', '-p', '-m', '777', dwld_path], 0, bash_cmd=True)
    run_rpmz_cmd(utils, ['reposync', '--download-path=' + dwld_path], 0)

    ARCH = platform.machine()
    rpm_dir = dwld_path + '/photon-test/RPMS/' + ARCH
    run_rpmz_cmd(utils, ['check-local', rpm_dir], 'ignore')

    run_bash_cmd(['rm', '-rf', dwld_path], 0, bash_cmd=True)


def test_repoquery(utils):
    run_rpmz_cmd(utils, ['repoquery', pkgname], 0)


def test_search(utils):
    run_rpmz_cmd(utils, ['search', pkgname], 0)


def test_update_info(utils):
    run_rpmz_cmd(utils, ['updateinfo'], 0)


def test_install(utils):
    run_rpmz_cmd(utils, ['install', '-y', pkgname], 1)


def test_erase(utils):
    run_rpmz_cmd(utils, ['erase', '-y', pkgname], 1)


def test_update(utils):
    run_rpmz_cmd(utils, ['update', '-y', pkgname], 1)
