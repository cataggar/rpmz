...
def setup_test(utils):
    ...
    # make sure to use the built rpmz binary, not the installed one by
...
def set_base_conf_and_store(fname):
    ini_set('base', 'rpmz_conf', rpmz_conf)
    ini_store(fname)
...
def test_rpmz_automatic_invalid_cfg_opt2(utils):
    ini_load(automatic_conf)
...
def test_rpmz_automatic_invalid_rpmz_conf(utils):
    ini_load(automatic_conf)
    ini_set('base', 'rpmz_conf', '/badpath/badfile.conf')
    ini_store(tmp_auto_conf)
    prepare_and_run_test_cmd(utils, [], 2, 'does not exist')


def test_rpmz_automatic_invalid_automatic_conf(utils):
...
def test_rpmz_automatic_disable_repos_retry(utils):
    repo_file = get_repo_file_path()
...
def test_rpmz_automatic_invalid_baseurl(utils):
    repo_file = get_repo_file_path()
    ini_load(repo_file)
...
def test_rpmz_automatic_show_updates(utils):
    cleanup_env(utils)
    pkgname = utils.config["mulversion_pkgname"]
    pkgversion = utils.config["mulversion_lower"]
    utils.run(['rpmz', 'install', '-y', pkgname + '-' + pkgversion])
...
def test_rpmz_automatic_apply_updates(utils):
    cleanup_env(utils)
    pkgname = utils.config["mulversion_pkgname"]
    pkgversion = utils.config["mulversion_lower"]
    utils.run(['rpmz', 'install', '-y', pkgname + '-' + pkgversion])
...
def test_rpmz_automatic_same_show_apply_updates(utils):
    cleanup_env(utils)
    pkgname = utils.config["mulversion_pkgname"]
    pkgversion = utils.config["mulversion_lower"]
    utils.run(['rpmz', 'install', '-y', pkgname + '-' + pkgversion])
...
def test_rpmz_automatic_hostname(utils):
    cleanup_env(utils)
    pkgname = utils.config["mulversion_pkgname"]
    pkgversion = utils.config["mulversion_lower"]
    utils.run(['rpmz', 'install', '-y', pkgname + '-' + pkgversion])
