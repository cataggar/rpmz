...
pub export fn TDNFGoal(arg_pTdnf: PTDNF, arg_pQueuePkgList: PTDNF_ID_LIST, arg_ppInfo: [*c]PTDNF_SOLVED_PKG_INFO, arg_nAlterType: TDNF_ALTERTYPE, arg_nUnresolved: c_int) u32 {
    ...
    dwError = tdnfSolv(pTdnf, &queueJobs, ppszExcludes, dwExcludeCount, nAllowErasing, @intFromBool(((pTdnf.*.pConf.*.nCleanRequirementsOnRemove != 0) and !(pTdnf.*.pArgs.*.nNoAutoRemove != 0)) or (nAlterType == @as(TDNF_ALTERTYPE, ALTER_AUTOERASE))), @intFromBool((nAlterType == @as(TDNF_ALTERTYPE, ALTER_REINSTALL)) or ((nAlterType == @as(TDNF_ALTERTYPE, ALTER_DISTRO_SYNC)) and (pTdnf.*.pConf.*.nDistroSyncReinstallChanged != 0))), nUnresolved, ppInfo);
    while (true) {
        if (dwError != @as(u32, 0)) return dwError;
        if (!false) break;
    }
...
fn captureSolved(
    ...
    var environment = transaction_plan_abi.IntegrationEnvironment{
        ...
        .force_architecture = tdnf.*.pArgs.*.pszArch,
        .excludes = @ptrCast(excludes),
        .installonly_names = @ptrCast(tdnf.*.pConf.*.ppszInstallOnlyPkgs),
        .locked_names = @ptrCast(tdnf.*.pConf.*.ppszPkgLocks),
        .min_versions = @ptrCast(tdnf.*.pConf.*.ppszMinVersions),
        .protected_names = @ptrCast(tdnf.*.pConf.*.ppszProtectedPkgs),
        .rpm_config = tdnf.*.pRpmConfig,
        .installonly_limit = @bitCast(tdnf.*.pConf.*.nInstallOnlyLimit),
        .allow_erasing = @intFromBool(allow_erasing != 0),
        .allow_multilib = 1,
        .all_deps = @intFromBool(tdnf.*.pArgs.*.nAllDeps != 0),
        .best = @intFromBool(tdnf.*.pArgs.*.nBest != 0),
        .clean_requirements_on_remove = @intFromBool(clean_deps != 0),
        .include_installed = @intFromBool(tdnf.*.pArgs.*.nAllDeps == 0),
        .install_weak_dependencies = 1,
        .keep_orphans = 1,
        .skip_broken = @intFromBool(tdnf.*.pArgs.*.nSkipBroken != 0),
    };

    var rc = TDNFTransactionPlanRequestTraceGetError(tdnf.*.pRequestTrace);
    if (rc != 0) {
        return rc;
    }
    rc = TDNFGoalBuildNativeSolverRepoInputs(tdnf, &repos, &repo_count);
    ...
    rc = TDNFTransactionPlanIntegrationCapturePending(
        tdnf.*.pTransactionPlanState,
        tdnf.*.pSack,
        native_solve,
        TDNFTransactionPlanRequestTraceGetView(tdnf.*.pRequestTrace),
        accepted,
        unresolved,
        terminal_problem,
        capture_repos,
...
fn captureFailure(
    ...
    var rc = TDNFGoalCaptureNativeSolve(tdnf, jobs, allow_erasing, auto_erase, flags, stamped_count, prepare_only, refute_unsat, drop_protected, native_solve);
    if (rc != 0) {
        TDNFTransactionPlanStateClear(tdnf.*.pTransactionPlanState);
        return original_error;
    }
    TDNFTransactionPlanRequestTraceFinalize(tdnf.*.pRequestTrace, jobs.*.pnElements, jobs.*.dwCount, TDNF_JOB_CLEANDEPS, TDNF_JOB_FORCEBEST);
    rc = captureSolved(tdnf, native_solve.*, 0, @bitCast(unresolved), terminal_problem, excludes, allow_erasing, auto_erase);
    if (rc != 0) TDNFTransactionPlanStateClear(tdnf.*.pTransactionPlanState);
    return original_error;
}
