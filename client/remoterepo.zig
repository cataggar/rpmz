...
fn pinParentUnderTrustedRoot(
    handle: *Tdnf,
    path: []const u8,
    create: bool,
    output: *PinnedParent,
) u32 {
    const conf = handle.pConf orelse return pinParent(path, create, output);
    const configured_root_z = cString(conf.pszCacheDir) orelse
        return pinParent(path, create, output);
    const configured_root = std.mem.span(configured_root_z);
    var root_end = configured_root.len;
    while (root_end > 1 and configured_root[root_end - 1] == '/') : (root_end -= 1) {}
    const root = configured_root[0..root_end];
    if (root.len == 0) return pinParent(path, create, output);
    const root_is_slash = std.mem.eql(u8, root, "/");
    if (path.len <= root.len or
        !std.mem.eql(u8, path[0..root.len], root) or
        (!root_is_slash and path[root.len] != '/'))
    {
        return pinParent(path, create, output);
    }
    const relative = path[root.len + @intFromBool(!root_is_slash) ..];
    ...
    var parent_fd: c_int = -1;
    result = openDirectoryComponents(root_fd, parent_path, create, &parent_fd);
    if (result != 0) return result;
    output.* = .{ .fd = parent_fd, .name = undefined };
    @memcpy(output.name[0..filename.len], filename);
    output.name[filename.len] = 0;
...
pub export fn TDNFDownloadFileFromRepo(
    ...
    const result = pinParentUnderTrustedRoot(
        handle,
        std.mem.span(file),
        false,
        &parent,
...
pub export fn TDNFDownloadPackageToCache(
    ...
    const conf = handle.pConf orelse return errors.ERROR_TDNF_INVALID_PARAMETER;
    const configured_cache = cString(conf.pszCacheDir) orelse
        return errors.ERROR_TDNF_INVALID_PARAMETER;
    const repo_id = cString(repo.pszId) orelse
        return errors.ERROR_TDNF_INVALID_PARAMETER;

    var cache_path: ?[*:0]u8 = null;
    var result = repoutils.RepoutilsGetRpmCachePath(
        handle,
        repo,
        &cache_path,
    );
...
fn downloadPackageToTreeAt(
    ...
    result = openDirectoryComponents(cache_root_fd, directory, true, &directory_fd);
    if (result != 0) return result;
...
test "configured cache and download root symlinks are resolved once" {
    ...
    defer std.testing.allocator.free(repository);
    try std.Io.Dir.cwd().createDirPath(std.testing.io, repository);
    const source = try std.fs.path.join(
        std.testing.allocator,
        &.{ repository, "example.rpm" },
    );
    defer std.testing.allocator.free(source);
    try writeTestFile(source, "symlinked root package");

    const real_cache = try std.fs.path.join(
        std.testing.allocator,
        &.{ root, "real-cache" },
    );
    defer std.testing.allocator.free(real_cache);
    const cache_link = try std.fs.path.join(
        std.testing.allocator,
        &.{ root, "cache-link" },
    );
    defer std.testing.allocator.free(cache_link);
    const real_download = try std.fs.path.join(
        std.testing.allocator,
        &.{ root, "real-download" },
    );
    defer std.testing.allocator.free(real_download);
    const download_link = try std.fs.path.join(
        std.testing.allocator,
        &.{ root, "download-link" },
    );
    defer std.testing.allocator.free(download_link);
    try std.Io.Dir.cwd().createDirPath(std.testing.io, real_cache);
    try std.Io.Dir.cwd().createDirPath(std.testing.io, real_download);
    ...
    const cache_z = try zString(cache_link);
    defer std.testing.allocator.free(cache_z);
    const download_z = try zString(download_link);
    defer std.testing.allocator.free(download_z);
    const source_url = try std.fmt.allocPrintSentinel(
        std.testing.allocator,
        "file://{s}",
        .{source},
    ...
    defer freeCString(cache_output);
    ...
    const metadata_dest = try std.fs.path.join(
        std.testing.allocator,
        &.{ cache_link, "repo-id", "repomd.xml" },
    );
    defer std.testing.allocator.free(metadata_dest);
    const metadata_dest_z = try zString(metadata_dest);
    defer std.testing.allocator.free(metadata_dest_z);
    try std.testing.expectEqual(
        @as(u32, 0),
        TDNFDownloadFile(
            &fixture.handle,
    ...
    const metadata_real = try std.fs.path.join(
        std.testing.allocator,
        &.{ real_cache, "repo-id", "repomd.xml" },
    );
    defer std.testing.allocator.free(metadata_real);
    const metadata = try readTestFile(metadata_real);
    defer std.testing.allocator.free(metadata);
    try std.testing.expectEqualStrings("symlinked root package", metadata);

    const repo_metadata_dest = try std.fs.path.join(
        std.testing.allocator,
        &.{ cache_link, "repo-id", "primary.xml" },
    );
    defer std.testing.allocator.free(repo_metadata_dest);
    const repo_metadata_dest_z = try zString(repo_metadata_dest);
    defer std.testing.allocator.free(repo_metadata_dest_z);
    try std.testing.expectEqual(
        @as(u32, 0),
    ...
    const escape = try std.fs.path.join(
        std.testing.allocator,
        &.{ real_cache, "escape" },
    );
    ...
    const escaped_dest = try std.fs.path.join(
        std.testing.allocator,
        &.{ cache_link, "escape", "repomd.xml" },
    );
    defer std.testing.allocator.free(escaped_dest);
    const escaped_dest_z = try zString(escaped_dest);
    defer std.testing.allocator.free(escaped_dest_z);
    try std.testing.expect(
        TDNFDownloadFile(
            &fixture.handle,
            &fixture.repo,
    ...
    var directory_output: ?[*:0]u8 = null;
    try std.testing.expectEqual(
        @as(u32, 0),
    ...
    defer freeCString(directory_output);
    const downloaded = try std.fs.path.join(
        std.testing.allocator,
        &.{ real_download, "example.rpm" },
    );
    defer std.testing.allocator.free(downloaded);
    const body = try readTestFile(downloaded);
    defer std.testing.allocator.free(body);
