...
/*
 * Version 7 additionally accepts the authoritative null-terminated protected
 * package-name array. A null pointer means no protected-package policy.
 * Authoritative protected-removal failures do not reach the observer.
 */
uint32_t
TDNFRepoMdNativeSolverLiveCompareV7(
    const TDNF_REPOMD_NATIVE_SOLVER_LIVE_REPOSITORY *pRepositories,
    uint32_t dwRepositoryCount,
    const TDNF_REPOMD_NATIVE_SOLVER_LIVE_JOB *pJobs,
    uint32_t dwJobCount,
...
/*
 * Version 9 additionally accepts authoritative allow-erasing policy. The live
 * observer admits available install jobs only when the legacy result has no
 * removals; conflict-removal and replacement shapes remain unsupported.
 */
uint32_t
TDNFRepoMdNativeSolverLiveCompareV9(
    const TDNF_REPOMD_NATIVE_SOLVER_LIVE_REPOSITORY *pRepositories,
    uint32_t dwRepositoryCount,
    const TDNF_REPOMD_NATIVE_SOLVER_LIVE_JOB *pJobs,
    uint32_t dwJobCount,
...
/*
 * Version 12 additionally accepts the null-terminated package names from
 * authoritative SOLVER_LOCK | SOLVER_SOLVABLE_NAME jobs. Locks require the
 * installed rpmdb; problem-bearing and unsupported result shapes stay closed.
 */
uint32_t
TDNFRepoMdNativeSolverLiveCompareV12(
    const TDNF_REPOMD_NATIVE_SOLVER_LIVE_REPOSITORY *pRepositories,
    uint32_t dwRepositoryCount,
    const TDNF_REPOMD_NATIVE_SOLVER_LIVE_JOB *pJobs,
    uint32_t dwJobCount,
...
/*
 * Version 13 additionally accepts the null-terminated package names from
 * authoritative SOLVER_MULTIVERSION | SOLVER_SOLVABLE_NAME jobs. Install-only
 * policy requires the installed rpmdb; limit-induced erases stay unsupported.
 */
uint32_t
TDNFRepoMdNativeSolverLiveCompareV13(
    const TDNF_REPOMD_NATIVE_SOLVER_LIVE_REPOSITORY *pRepositories,
    uint32_t dwRepositoryCount,
    const TDNF_REPOMD_NATIVE_SOLVER_LIVE_JOB *pJobs,
    uint32_t dwJobCount,
...
/*
 * Version 14 additionally accepts the null-terminated package names that the
 * user asked for explicitly, mirroring the libsolv SOLVER_USERINSTALLED feed
 * derived from the history auto flags. Installed packages absent from the list
 * are treated as dependency-installed and are therefore clean-deps eligible.
 * Passing NULL leaves every install reason unknown, which keeps all installed
 * packages as clean-deps roots exactly like versions 2 through 13.
 */
uint32_t
TDNFRepoMdNativeSolverLiveCompareV14(
    const TDNF_REPOMD_NATIVE_SOLVER_LIVE_REPOSITORY *pRepositories,
    uint32_t dwRepositoryCount,
    const TDNF_REPOMD_NATIVE_SOLVER_LIVE_JOB *pJobs,
    uint32_t dwJobCount,
