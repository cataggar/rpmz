...
fn parseCommand(pHistoryArgs: *abi.TDNF_HISTORY_ARGS, pszCommand: ?[*:0]const u8) void {
    if (equals(pszCommand, "list")) {
        pHistoryArgs.nCommand = abi.HISTORY_CMD_LIST;
    } else if (equals(pszCommand, "init") or equals(pszCommand, "update")) {
        pHistoryArgs.nCommand = abi.HISTORY_CMD_INIT;
    } else if (equals(pszCommand, "rollback")) {
        pHistoryArgs.nCommand = abi.HISTORY_CMD_ROLLBACK;
    } else if (equals(pszCommand, "undo")) {
        pHistoryArgs.nCommand = abi.HISTORY_CMD_UNDO;
    } else if (equals(pszCommand, "redo")) {
        pHistoryArgs.nCommand = abi.HISTORY_CMD_REDO;
    } else if (equals(pszCommand, "id")) {
        pHistoryArgs.nCommand = abi.HISTORY_CMD_ID;
    }
}

fn parseRange(pHistoryArgs: *abi.TDNF_HISTORY_ARGS, pszRange: ?[*:0]const u8) void {
...
fn parseSetOpts(pHistoryArgs: *abi.TDNF_HISTORY_ARGS, pSetOpts: [*c]abi.struct_cnfnode) void {
    var pNode: [*c]abi.struct_cnfnode = if (pSetOpts != null) pSetOpts[0].first_child else null;
...
pub export fn TDNFCliParseHistoryArgs(
    ...
    ppHistoryArgs: ?*?*abi.TDNF_HISTORY_ARGS,
) u32 {
    const cmd_args = pArgs orelse return abi.ERROR_TDNF_CLI_INVALID_ARGUMENT;
    const out = ppHistoryArgs orelse return abi.ERROR_TDNF_CLI_INVALID_ARGUMENT;

    const pAllocated = c.calloc(1, @sizeOf(abi.TDNF_HISTORY_ARGS)) orelse
        return abi.ERROR_TDNF_OUT_OF_MEMORY;
    const pHistoryArgs: *abi.TDNF_HISTORY_ARGS = @ptrCast(@alignCast(pAllocated));
...
pub export fn TDNFCliFreeHistoryArgs(pHistoryArgs: ?*abi.TDNF_HISTORY_ARGS) void {
...
test "TDNFCliParseHistoryArgs preserves history command and range parsing" {
    ...
    var from_node: abi.struct_cnfnode = std.mem.zeroes(abi.struct_cnfnode);
    var info_node: abi.struct_cnfnode = std.mem.zeroes(abi.struct_cnfnode);
    ...
    try std.testing.expectEqual(
        @as(@TypeOf(pHistoryArgs.?.nCommand), @intCast(abi.HISTORY_CMD_ROLLBACK)),
...
test "TDNFCliParseHistoryArgs defaults missing to range end" {
    var setopt_root: abi.struct_cnfnode = std.mem.zeroes(abi.struct_cnfnode);
    var reverse_node: abi.struct_cnfnode = std.mem.zeroes(abi.struct_cnfnode);
    ...
    try std.testing.expect(pHistoryArgs != null);
    try std.testing.expectEqual(
        @as(@TypeOf(pHistoryArgs.?.nCommand), @intCast(abi.HISTORY_CMD_ID)),
        pHistoryArgs.?.nCommand,
    );
