...
Use `-Doptimize=ReleaseSafe`, as CI does. The audits inspect whatever is
installed in the prefix, so running them bare rebuilds the tree as Debug
and overwrites a ReleaseSafe install. `abi-audit` then fails: zig links
`libtdnf.so` with its self-hosted ELF linker in Debug builds and does not
forward `--version-script` to it, so `client/libtdnf.map` is silently
skipped and the vendored libsolv and SQLite symbols it hides reappear.
