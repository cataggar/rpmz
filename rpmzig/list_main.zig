...
pub fn main(init: std.process.Init) u8 {
    ...
    const iterator = rpmdb.Iter.openRoot(root) catch |err| {
        std.debug.print("rpmz-rpmdb-list: open: rpmdb_iter_open: {t}\n", .{err});
        return 1;
    };
    ...
    while (true) {
        const package_header = iterator.nextHeader() catch |err| {
            std.debug.print("rpmz-rpmdb-list: iter.nextHeader: {t}\n", .{err});
            return 1;
        } orelse break;
        const nevra = package_header.allocNevra(std.heap.c_allocator) catch {
            std.debug.print("rpmz-rpmdb-list: out of memory building NEVRA\n", .{});
            return 1;
        } orelse {
            std.debug.print("rpmz-rpmdb-list: header missing required tag for NEVRA\n", .{});
            return 1;
        };
