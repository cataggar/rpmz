/*
 * rpmz rpmdb (Zig) — private ABI for the Zig rpmdb reader.
 *
 ...
 * decoder) and is linked into rpmz, rpmz-rpmdb-count, and
 * rpmz-rpmdb-list.
 */
...
typedef struct rpmz_rpm_file rpmz_rpm_file;
typedef struct rpmz_rpm_config rpmz_rpm_config;
 ...
 * rpmz_rpmdb_last_error() to retrieve the error message.
 */
int64_t rpmz_rpmdb_count_packages(const char *root);
int64_t rpmz_rpmdb_count_packages_config(
    const rpmz_rpm_config *config);
 ...
 */
const char *rpmz_rpmdb_last_error(void);
 ...
 *
 * Returns NULL on error (use rpmz_rpmdb_last_error()).
 */
rpmz_rpm_config *rpmz_rpm_config_create(const char *install_root);

/**
 * Destroy a configuration created by rpmz_rpm_config_create().
 * Accepts NULL.
 */
void rpmz_rpm_config_destroy(rpmz_rpm_config *config);

char *rpmz_rpm_config_install_root(const rpmz_rpm_config *config);
int rpmz_rpm_config_open_root_fd(const rpmz_rpm_config *config);
 ...
 */
int rpmz_rpm_config_apply_define(
    rpmz_rpm_config *config,
 ...
 * rpmz_rpm_config_string_free(). Returns NULL on error.
 */
char *rpmz_rpm_config_expand(
    const rpmz_rpm_config *config,
    const char *name
);
 ...
 * and must free it with rpmz_rpm_config_string_free().
 */
char *rpmz_rpm_config_resolve_path(
    const rpmz_rpm_config *config,
    const char *name
);
 ...
 */
char *rpmz_rpmdb_cookie(const char *root);

char *rpmz_rpmdb_cookie_config(
    const rpmz_rpm_config *config
);

/* --- forward iterator over the Packages table --- */

typedef struct rpmz_rpmdb_iter rpmz_rpmdb_iter;
 ...
 * Accepts NULL.
 */
void rpmz_rpmdb_iter_close(rpmz_rpmdb_iter *it);
 ...
 *   -1 on error (use rpmz_rpmdb_last_error).
 */
int rpmz_rpmdb_iter_next_nevra(rpmz_rpmdb_iter *it, char **nevra_out);
 ...
 * valid until the next iterator advance or `rpmz_rpmdb_iter_close`.
 ...
 *   -1 on error (use rpmz_rpmdb_last_error).
 */
int rpmz_rpmdb_iter_next_header_blob(
    rpmz_rpmdb_iter *it,
 ...
 * rpmz_rpmdb_iter_next_header_blob(). `*hnum_out` identifies the exact
 * Packages row represented by the returned blob.
 */
int rpmz_rpmdb_iter_next_header_blob_hnum(
    rpmz_rpmdb_iter *it,
    uint32_t *hnum_out,
    const unsigned char **blob_out,
    size_t *blob_len_out
...
/**
 * Free a string returned by rpmz_rpmdb_iter_next_nevra. Accepts NULL.
 */
void rpmz_rpmdb_string_free(char *s);
 ...
 * is allocated on success and must be freed with rpmz_rpmdb_string_free.
 *
 * Returns:
 ...
 */
int rpmz_rpmdb_resolve_provider_version_config(
    const rpmz_rpm_config *config,
...
int rpmz_rpmdb_write_install_config(
    const rpmz_rpm_config *config,
    const char *rpm_path,
    uint32_t install_tid,
    uint32_t install_time,
 ...
 */
int rpmz_rpmdb_write_install_file_config(
    const rpmz_rpm_config *config,
    rpmz_rpm_file *fh,
    uint32_t install_tid,
    uint32_t install_time,
...
int rpmz_rpmdb_write_replace_config(
    const rpmz_rpm_config *config,
    uint32_t old_hnum,
    const char *rpm_path,
    uint32_t install_tid,
 ...
 */
int rpmz_rpmdb_write_replace_file_config(
    const rpmz_rpm_config *config,
    uint32_t old_hnum,
    rpmz_rpm_file *fh,
    uint32_t install_tid,
 ...
 * Returns 0 on success, -1 on error.
 */
int rpmz_rpmdb_write_erase_hnum(const char *root, uint32_t hnum);
int rpmz_rpmdb_write_erase_hnum_config(
    const rpmz_rpm_config *config,
    uint32_t hnum
);
 ...
 *   -1 on error
 */
int rpmz_rpmdb_find_hnum_by_nevra(
    const char *root,
    const char *nevra,
    uint32_t *hnum_out
);
int rpmz_rpmdb_find_hnum_by_nevra_config(
    const rpmz_rpm_config *config,
    const char *nevra,
    uint32_t *hnum_out
);
 ...
 * `*hnums_out` (free with rpmz_rpmdb_hnums_free) and the count into
 * `*count_out`. When there are no matches, `*hnums_out` is NULL and
 * `*count_out` is 0.
 *
 * Returns 0 on success, -1 on error (use rpmz_rpmdb_last_error()).
 */
int rpmz_rpmdb_find_hnums_by_name(
    const char *root,
    const char *name,
    uint32_t **hnums_out,
    size_t *count_out
);
int rpmz_rpmdb_find_hnums_by_name_config(
    const rpmz_rpm_config *config,
    const char *name,
    uint32_t **hnums_out,
    size_t *count_out
...
typedef struct rpmz_rpmdb_label_match {
    uint32_t hnum;
    char *name;
    char *evr;
    char *arch;
} rpmz_rpmdb_label_match;
 ...
 * rpmz_rpmdb_label_matches_free().
 */
int rpmz_rpmdb_find_label_matches_config(
    const rpmz_rpm_config *config,
    const char *name,
    const char *evr,
    rpmz_rpmdb_label_match **matches_out,
    size_t *count_out
);

void rpmz_rpmdb_label_matches_free(
    rpmz_rpmdb_label_match *matches,
    size_t count
);
 ...
 * Accepts NULL.
 */
void rpmz_rpmdb_hnums_free(uint32_t *hnums);

/**
 * Read the raw main-header blob for the installed package with
 * `hnum` under `root`. On success writes a heap-allocated blob
 * pointer into `*blob_out` (free with rpmz_rpmdb_blob_free) and its
 * length into `*len_out`.
 *
 * Returns 0 on success, -1 on error (use rpmz_rpmdb_last_error()).
 */
int rpmz_rpmdb_read_header_blob(
    const char *root,
    uint32_t hnum,
    unsigned char **blob_out,
    size_t *len_out
);
int rpmz_rpmdb_read_header_blob_config(
    const rpmz_rpm_config *config,
    uint32_t hnum,
    unsigned char **blob_out,
    size_t *len_out
...
/**
 * Free a blob returned by rpmz_rpmdb_read_header_blob. Accepts NULL.
 */
void rpmz_rpmdb_blob_free(unsigned char *blob);

/* --- pubkey iterator (rpmdb-resident gpg-pubkey-* entries) --- */

typedef struct rpmz_rpmdb_pubkeys_iter rpmz_rpmdb_pubkeys_iter;
 ...
 */
int rpmz_rpmdb_import_pubkeys(
    const char *root,
    const void *data,
    size_t len,
    size_t *imported_out
);
int rpmz_rpmdb_import_pubkeys_config(
    const rpmz_rpm_config *config,
    const void *data,
    size_t len,
    size_t *imported_out
 ...
 */
rpmz_rpmdb_pubkeys_iter *rpmz_rpmdb_pubkeys_open(const char *root);
rpmz_rpmdb_pubkeys_iter *rpmz_rpmdb_pubkeys_open_config(
    const rpmz_rpm_config *config
);
 ...
 */
void rpmz_rpmdb_pubkeys_close(rpmz_rpmdb_pubkeys_iter *it);
 ...
 * rpmz_rpmdb_string_free) and optionally its length (excluding
 ...
 *   -1 on error (use rpmz_rpmdb_last_error).
 */
int rpmz_rpmdb_pubkeys_next(
    rpmz_rpmdb_pubkeys_iter *it,
    char **key_out,
    size_t *key_len_out,
    char **keyid_out
);
 ...
 */
rpmz_rpm_file *rpmz_rpm_file_open(const char *path);

/**
 * Free a file handle. Accepts NULL.
 */
void rpmz_rpm_file_close(rpmz_rpm_file *fh);

/**
 * Returns a heap-allocated NEVRA string for this rpm file. Caller
 * frees with rpmz_rpmdb_string_free.
 */
char *rpmz_rpm_file_nevra(rpmz_rpm_file *fh);
 ...
 */
int rpmz_rpm_file_package_kind(rpmz_rpm_file *fh);

typedef struct rpmz_rpm_file_metadata {
    const char *name;
    const char *version;
    const char *release;
    const char *arch;
...
} rpmz_rpm_file_metadata;

typedef struct rpmz_rpm_header_view {
    const unsigned char *blob;
    size_t len;
} rpmz_rpm_header_view;
 ...
 */
int rpmz_rpm_header_name_equals(
    const unsigned char *header_blob,
    size_t header_len,
    const char *name
);

int rpmz_rpm_header_owns_path(
    const unsigned char *header_blob,
    size_t header_len,
    const char *path
);

int rpmz_rpm_canonical_path_config(
    const rpmz_rpm_config *config,
    const char *path,
    char *output,
    size_t output_len
...
int rpmz_rpm_header_owns_path_config(
    const unsigned char *header_blob,
    size_t header_len,
    const char *path,
    const rpmz_rpm_config *config
 ...
 * rpmz_rpm_file_close(). No pathname is reopened and no librpm Header is
 * constructed.
 */
int rpmz_rpm_file_get_metadata(
    rpmz_rpm_file *fh,
    rpmz_rpm_file_metadata *metadata_out
);
 ...
 * rpmz_rpm_file_close(). Returns 0 on success, -1 on invalid arguments.
 */
int rpmz_rpm_file_bytes(
    rpmz_rpm_file *fh,
    const unsigned char **out,
    size_t *out_len
);
 ...
 */
const char *rpmz_rpm_file_compressor(rpmz_rpm_file *fh);

/**
 * Returns the byte offset of the payload (cpio archive) within the
 * underlying file.
 */
int64_t rpmz_rpm_file_payload_offset(rpmz_rpm_file *fh);
 ...
 */
int rpmz_rpm_file_is_signed(rpmz_rpm_file *fh);
 ...
 * static; do not free.
 */
const char *rpmz_rpm_file_signature_kind(rpmz_rpm_file *fh);
 ...
 * access failure, or allocation failure); consult rpmz_rpmdb_last_error().
 */
enum {
 ...
 * required.
 */
int rpmz_rpm_file_verify_digests(
    rpmz_rpm_file *fh,
    int *outcome_out
);
 ...
 */
int rpmz_rpm_file_verify_signatures_config(
    rpmz_rpm_file *fh,
    const rpmz_rpm_config *config,
 ...
 */
int rpmz_rpm_file_verify_signatures_keys(
    rpmz_rpm_file *fh,
 ...
 * Returns 0 on success, -1 on error (use rpmz_rpmdb_last_error).
 */
int rpmz_rpm_file_decompress_payload(
    rpmz_rpm_file *fh,
    unsigned char **out,
    size_t *out_size
);
 ...
 */
int rpmz_rpm_file_extract_source_config(
    rpmz_rpm_file *fh,
    const rpmz_rpm_config *config,
...
typedef enum rpmz_rpm_install_kind {
    TDNF_RPM_INSTALL_KIND_INSTALL = 0,
    TDNF_RPM_INSTALL_KIND_UPGRADE = 1,
    TDNF_RPM_INSTALL_KIND_REINSTALL = 2,
} rpmz_rpm_install_kind;

typedef struct rpmz_rpm_install_prior_header {
    const unsigned char *blob;
    size_t len;
} rpmz_rpm_install_prior_header;

/**
 * Callback used by rpmz_rpm_file_install() to ask whether `path`
 ...
 * opaque pointer from rpmz_rpm_install_options.conflict_fn_data.
 */
typedef int (*rpmz_rpm_install_conflict_fn)(void *data, const char *path);
typedef int (*rpmz_rpm_changed_path_fn)(void *data, const char *path);

typedef struct rpmz_rpm_install_options {
    /**
     ...
     */
    rpmz_rpm_install_kind install_kind;
    /**
     ...
     * rows or rpmz_rpm_file_main_header_blob().
     */
    const rpmz_rpm_install_prior_header *prior_headers;
     ...
     */
    rpmz_rpm_install_conflict_fn conflict_fn;
     ...
     */
    rpmz_rpm_changed_path_fn changed_path_fn;
    void *changed_path_fn_data;
} rpmz_rpm_install_options;
 ...
 * Returns 0 on success, -1 on error (use rpmz_rpmdb_last_error()).
 */
int rpmz_rpm_file_install(
    rpmz_rpm_file *fh,
    const rpmz_rpm_install_options *options
);

/* --- native scriptlet execution primitive (T4 building block) --- */

typedef enum rpmz_rpm_scriptlet_phase {
...
} rpmz_rpm_scriptlet_phase;

typedef enum rpmz_rpm_scriptlet_outcome {
...
} rpmz_rpm_scriptlet_outcome;

typedef struct rpmz_rpm_scriptlet_options {
    /**
     ...
     * onto stdout before exec. This matches the rpmz JSON-output
     * requirement where scriptlet chatter must not corrupt stdout.
     */
    int redirect_stdout_to_stderr;
} rpmz_rpm_scriptlet_options;

typedef struct rpmz_rpm_scriptlet_result {
    int ran;
    int critical;
    rpmz_rpm_scriptlet_outcome outcome;
    int exit_status;
    int signal_number;
} rpmz_rpm_scriptlet_result;
 ...
 * rpmz_rpm_file_main_header_blob() or stored in the sqlite rpmdb
 ...
 * rpmz_rpmdb_last_error()).
 */
int rpmz_rpm_header_run_scriptlet(
    const unsigned char *header_blob,
    size_t header_len,
    rpmz_rpm_scriptlet_phase phase,
    const rpmz_rpm_scriptlet_options *options,
    rpmz_rpm_scriptlet_result *result_out
);

/* --- native trigger execution engine (T4 building block) --- */

typedef enum rpmz_rpm_trigger_phase {
    TDNF_RPM_TRIGGER_PHASE_TRIGGERIN = 0,
    TDNF_RPM_TRIGGER_PHASE_TRIGGERUN = 1,
    TDNF_RPM_TRIGGER_PHASE_TRIGGERPOSTUN = 2,
} rpmz_rpm_trigger_phase;

typedef struct rpmz_rpm_trigger_options {
    /**
     ...
     */
    const rpmz_rpm_header_view *transaction_headers;
     ...
     */
    const rpmz_rpm_header_view *trigger_owner_headers;
    size_t trigger_owner_header_count;
    int trigger_owner_view_present;
} rpmz_rpm_trigger_options;

typedef struct rpmz_rpm_trigger_result {
    int ran;
    int critical;
    rpmz_rpm_scriptlet_outcome outcome;
    int exit_status;
    int signal_number;
} rpmz_rpm_trigger_result;
 ...
 * API/setup/parse failures return -1 (use rpmz_rpmdb_last_error()).
 */
int rpmz_rpm_header_run_triggers(
    const unsigned char *header_blob,
    size_t header_len,
    rpmz_rpm_trigger_phase phase,
    const rpmz_rpm_trigger_options *options,
    rpmz_rpm_trigger_result *result_out
);

typedef enum rpmz_rpm_file_trigger_kind {
    TDNF_RPM_FILE_TRIGGER_KIND_PACKAGE = 0,
    TDNF_RPM_FILE_TRIGGER_KIND_TRANSACTION = 1,
} rpmz_rpm_file_trigger_kind;

typedef enum rpmz_rpm_trigger_priority_class {
    TDNF_RPM_TRIGGER_PRIORITY_ALL = 0,
    TDNF_RPM_TRIGGER_PRIORITY_HIGH = 1,
    TDNF_RPM_TRIGGER_PRIORITY_LOW = 2,
} rpmz_rpm_trigger_priority_class;

typedef struct rpmz_rpm_trigger_path {
    const char *path;
    const unsigned char *source_header_blob;
    size_t source_header_len;
} rpmz_rpm_trigger_path;

typedef struct rpmz_rpm_file_trigger_owner {
    const unsigned char *header_blob;
    size_t header_len;
    const rpmz_rpm_trigger_path *paths;
    size_t path_count;
    uint64_t order;
} rpmz_rpm_file_trigger_owner;

typedef struct rpmz_rpm_file_trigger_options {
    const char *install_root;
    const rpmz_rpm_config *config;
    int install_root_fd;
    uint32_t trans_flags;
 ...
 */
int rpmz_rpm_header_validate_trigger_metadata(
    const unsigned char *header_blob,
    size_t header_len
);
 ...
 */
int rpmz_rpm_header_validate_trigger_scripts_config(
    const unsigned char *header_blob,
    size_t header_len,
    const rpmz_rpm_config *config
);
 ...
 */
int rpmz_rpm_header_has_file_trigger_metadata(
    const unsigned char *header_blob,
    size_t header_len,
    rpmz_rpm_file_trigger_kind kind
);
 ...
 */
int rpmz_rpm_header_foreach_trigger_file(
    const unsigned char *header_blob,
    size_t header_len,
    uint32_t trans_flags,
    rpmz_rpm_changed_path_fn callback,
    void *callback_data
);
 ...
 */
int rpmz_rpm_run_file_triggers(
    const rpmz_rpm_file_trigger_owner *owners,
    size_t owner_count,
    rpmz_rpm_trigger_phase phase,
    rpmz_rpm_file_trigger_kind kind,
    rpmz_rpm_trigger_priority_class priority_class,
    const rpmz_rpm_file_trigger_options *options,
    rpmz_rpm_trigger_result *result_out
);
...
/**
 * Callback used by rpmz_rpm_erase_hnum() to decide whether `path`
 ...
 * pointer from rpmz_rpm_erase_options.keep_path_fn_data.
 *
 ...
 */
typedef int (*rpmz_rpm_erase_keep_path_fn)(void *data, const char *path);

typedef struct rpmz_rpm_erase_options {
    /**
     * Optional handle-scoped rpm configuration. When supplied it is
     * authoritative for the rpmdb, install root, and native macros.
     */
     ...
     * query.
     */
    rpmz_rpm_erase_keep_path_fn keep_path_fn;
    void *keep_path_fn_data;
} rpmz_rpm_erase_options;
 ...
 * rpmz_rpmdb_write_erase_hnum() to match real rpm's ordering.
 *
 * `options` may be NULL for defaults.
 *
 * Returns 0 on success, -1 on error (use rpmz_rpmdb_last_error()).
 */
int rpmz_rpm_erase_hnum(
    const char *root,
    uint32_t hnum,
    const rpmz_rpm_erase_options *options
);
 ...
 *
 * Unlike rpmz_rpm_erase_hnum(), the header is supplied directly by
 * the caller. This is used on the UPGRADE path: after
 * rpmz_rpmdb_write_replace() has atomically overwritten the OLD
 ...
 * Returns 0 on success, -1 on error (use rpmz_rpmdb_last_error()).
 */
int rpmz_rpm_erase_header_blob(
    const char *root,
    const unsigned char *blob,
    size_t blob_len,
    const rpmz_rpm_erase_options *options
...
typedef struct rpmz_rpm_files_iter rpmz_rpm_files_iter;
 ...
 * Returns NULL on error (use rpmz_rpmdb_last_error).
 */
rpmz_rpm_files_iter *rpmz_rpm_file_files_open(rpmz_rpm_file *fh);

/**
 * Free a files iterator. Accepts NULL.
 */
void rpmz_rpm_file_files_close(rpmz_rpm_files_iter *it);
 ...
 *   -1 on error.
 */
int rpmz_rpm_file_files_next(
    rpmz_rpm_files_iter *it,
    const char **name_out,
    uint32_t *mode_out
);
 ...
 */
int rpmz_rpm_file_signed_range(
    rpmz_rpm_file *fh,
